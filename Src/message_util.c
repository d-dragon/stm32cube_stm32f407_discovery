...
uint16_t Calulate_CRC(uint8_t *data, uint8_t length)
{
	uint8_t data_temp[length + 2];
	uint8_t i;
	uint16_t crc;
//	memcpy(data_temp, data, (size_t)length);
...
uint8_t MatLab_Message_Parser(MatLab_Message_TypeDef *ml_msg, uint8_t *serial_data, uint8_t data_len) {
	...
	uint16_t crc, msg_crc;

	/********* Validate CRC  **********/
	crc = Calulate_CRC(serial_data + 1, data_len - 3);
	msg_crc = (uint16_t)serial_data[data_len - 1];
	msg_crc = msg_crc << 8;
	msg_crc |= (uint16_t) serial_data[data_len - 2];

	if (crc != msg_crc) {
		return MSG_PARSER_CRC_ERROR;
	}
...
uint8_t MatLab_Send_Response(uint8_t msg_type, uint8_t *payload, uint8_t payload_len) {
	...
	uint8_t *data = &res_msg[1];
	crc = Calulate_CRC(data, MSG_TYPE_LEN + payload_len);

	res_msg[msg_len - 2] = (uint8_t) (0x00ff & crc);
	res_msg[msg_len - 1] = (uint8_t) (0x00ff & (crc >> 8));
	/*******************************/

	HAL_UART_Transmit(&huart3, res_msg, msg_len, 5);
//	HAL_UART_Transmit(&huart1, res_msg, msg_len, 5);
	return 0;
}
